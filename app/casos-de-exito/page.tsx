            ...
            {CLIENTS.map((c) => (
              ...
              >
                {c.logoSlug ? (
                  ...
                  <p className="font-rpc-heading text-xl uppercase tracking-[0.1em] text-rpc-text sm:text-2xl">
                    {c.name}
                  </p>
                )}
              </article>
      ...
      <section className="border-b border-rpc-border bg-rpc-bg">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10 lg:py-24">
          ...
          <dl className="mt-12 grid gap-x-10 gap-y-12 sm:grid-cols-2 lg:gap-y-16">
            {VALUE_PROPS.map((v, i) => (
              <div key={v.title} className="border-t border-rpc-border pt-6">
                <div className="flex items-start gap-4">
                  {/* Icon en círculo: ancla visual + número de orden al lado */}
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-rpc-text/30 text-rpc-text">
                    {v.icon}
                  </div>
                  <p className="font-rpc-heading text-3xl font-light leading-none text-rpc-text/30 sm:text-4xl">
                    {String(i + 1).padStart(2, "0")}
                  </p>
                </div>
                <dt className="mt-5 font-rpc-heading text-lg uppercase tracking-[0.08em] text-rpc-text sm:text-xl">
                  {v.title}
                </dt>
                <dd className="mt-3 max-w-prose font-rpc-body text-sm normal-case tracking-normal text-rpc-text/75 sm:text-base">
                  {v.body}
                </dd>
              </div>
            ))}
