...
> = {
  ...
  INFEASIBLE: {
    ...
    dot: "bg-rpc-error",
    pill: "border-rpc-error/40 bg-rpc-error/10 text-rpc-error",
  },
};
...
export function StockAnalysis({ analysis, requiredDate }: Props) {
  ...
  return (
    <section className="rounded-rpc-card border border-rpc-border bg-rpc-bg p-6">
      <header className="flex items-center justify-between gap-3">
        ...
        <span
          className={cn(
            "inline-flex items-center gap-2 rounded-rpc-button border px-3 py-1.5 text-[10px] uppercase tracking-[0.18em]",
            meta.pill,
          )}
        >
      ...
      <p className="mt-4 font-rpc-body text-sm normal-case tracking-normal leading-relaxed text-rpc-text">
...
function Metric({
  ...
  return (
    <div>
      <dt className="text-[10px] uppercase tracking-[0.18em] text-rpc-text/50">
        {label}
      </dt>
      <dd
        className={cn(
          "mt-1 font-rpc-heading text-base font-light",
          emphasize ? "text-rpc-error" : "text-rpc-text",
        )}
      >
        {value}
      </dd>
...
function Gantt({
  ...
  return (
    <div className="mt-6">
      <div className="relative h-12 w-full rounded-sm bg-rpc-image-bg-light">
        {segments.map((s, i) => {
        ...
        <div
          className="absolute -top-1 -bottom-1 w-px bg-rpc-error"
          style={{ left: `${pct(required)}%` }}
          aria-hidden
        />
        <span
          className="absolute -top-5 -translate-x-1/2 text-[9px] uppercase tracking-[0.15em] text-rpc-error"
          style={{ left: `${pct(required)}%` }}
        >
          Tu fecha
        </span>
      ...
      {/* Leyenda */}
      <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-[10px] uppercase tracking-[0.18em] text-rpc-text/60">
