        ...
        return (
          <div key={optName}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="font-rpc-body text-sm normal-case tracking-normal text-rpc-text">
                {selectedValue ?? "—"}
              </span>
            </div>
              ...
              {values.map((value) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "rounded-rpc-button border px-4 py-2 text-xs uppercase tracking-[0.18em] transition",
                      isActive
                        ? "border-rpc-text bg-rpc-text text-rpc-button-text"
                        : "border-rpc-border bg-rpc-bg text-rpc-text hover:border-rpc-text",
                    )}
                  >
                    {value}
                  </button>
