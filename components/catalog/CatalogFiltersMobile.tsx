  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        className={cn(
          "flex w-full items-center justify-between gap-3 rounded-rpc-button border border-rpc-border bg-rpc-bg px-4 py-3 text-left transition",
          open && "border-rpc-text",
        )}
        aria-expanded={open}
      >
        <span className="flex items-center gap-3">
          ...
          <span className="text-xs uppercase tracking-[0.2em] text-rpc-text">
            Filtros
          </span>
          {activeCount > 0 && (
            <span className="rounded-full bg-rpc-text px-2 py-0.5 text-[10px] font-medium text-rpc-button-text">
              {activeCount}
            </span>
          )}
        </span>
        <span className="font-rpc-body text-xs tracking-normal text-rpc-text/60">
          {filteredCount} de {totalProducts}
        </span>
      </button>

      {open && (
        <div className="mt-4 rounded-rpc-card border border-rpc-border bg-rpc-bg p-5">
          {children}
        </div>
      )}
    </div>
